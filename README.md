...
**한국어와 영어를 한 번에 전환**할 수 있고, **46장을 그대로 PDF로 내려받을** 수 있다.
...
`←` `→` `Space` 이동 · `O` 전체 개요 · `L` 한/영 전환 · `P` 인쇄·PDF 모드 ·
...
1. **미리 구운 PDF** — 위 표의 링크. 46쪽, 한 쪽이 슬라이드 한 장(1280×720 가로).
2. **브라우저에서 직접** — `P` 키를 누르면 46장이 지면에 펼쳐진다. `Ctrl+P` → PDF로 저장.
   인쇄 설정에서 **배경 그래픽**을 켜야 어두운 배경이 그대로 나온다.
