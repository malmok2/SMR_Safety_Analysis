...
- **제목은 그 장이 무엇을 다루는지 그대로 쓴다.** 강의 노트이지 카피가 아니다.
  처음에는 제목을 경구처럼 달았다가 전부 걷어냈다 — 학생이 제목만 보고 무슨 장인지
  알 수 없으면 목차도 쓸모가 없어진다.

  | 쓰지 않는다 | 쓴다 |
  |---|---|
  | 유량을 2배로 늘리려면 높이를 4배로 벌려야 함 | 자연순환 유량과 <em>용기 높이</em> |
  | 잃은 물이 격납 안에 그대로 있으므로 되돌리기만 하면 됨 | <em>ECCS</em> — RVV 방출과 RRV 회수 |
  | "72시간 무조작"은 마법이 아니라 물통 크기를 정하는 계산이다 | 무조작 <em>72시간</em>에 필요한 수조 용량 |

  명사구나 평서문으로 쓰고, 수사의문문·반전·따옴표 제목·`<s>` 취소선을 쓰지 않는다.
  핵심 용어 하나에만 `<em>`을 걸어 강조색을 준다. 주장은 제목이 아니라 **불릿**에 쓴다.
- **레일 태그(`tag`)와 개요 라벨(`ov`)도 같은 원칙**이다. `ov` 는 제목과 같게 둔다 —
  `O` 개요 화면이 곧 목차이므로 제목과 다르면 학생이 두 번 읽어야 한다.
